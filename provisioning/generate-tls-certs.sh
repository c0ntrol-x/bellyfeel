...
signed_cert_filename="${file_prefix}.${DOMAIN}.signed.cert"
...
if python "${HERE}/sign_csr.py" --email="${EMAIL}" --public-key "${public_key_filename}" "${csr_filename}" > "${signed_cert_filename}"; then
...
cat "${signed_cert_filename}" lets-encrypt-x3-cross-signed.pem > "${cert_filename}"
